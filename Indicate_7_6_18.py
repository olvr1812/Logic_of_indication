...
class app_7_6_18:
    ...
    im = config_path + '/Images/Indication_7_6_20/Var1.png'




    logic = open(config_path + '/logic_text/logic_7_6_18.txt', encoding='utf-8').readlines()
    logic = ''.join(logic)
    ...
    def chek_b1(self):
        v_s = []
        for l in range(len(app_7_6_18.descriptSig)):
            exec('v_s.append(self.var{}.get())'.format(str(l)))
        print(v_s, sep="\n")

        self.im = app_7_6_18.logik_of_indication(self, v_s)
        ...
        self.canvas.create_image(9, 9, image=self.photo, anchor=NW)

    def logik_of_indication(self, v_s):
        if v_s[0] == 1:
            im = app_7_6_18.config_path + "/Images/Indication_7_6_18/Var1.png"
            self.result_lbl.config(text="Вариант 1", foreground="black")
        elif v_s[4] == 1:
            im = app_7_6_18.config_path + "/Images/Indication_7_6_18/Var5.png"
            self.result_lbl.config(text="Вариант 5", foreground="black")
        elif v_s[1] == 1:
            im = app_7_6_18.config_path + "/Images/Indication_7_6_18/Var2.png"
            self.result_lbl.config(text="Вариант 2", foreground="black")
        elif v_s[5] == 1:
            im = app_7_6_18.config_path + "/Images/Indication_7_6_18/Var6.png"
            self.result_lbl.config(text="Вариант 6", foreground="black")
        elif v_s[2] == 1:
            im = app_7_6_18.config_path + "/Images/Indication_7_6_18/Var3.png"
            self.result_lbl.config(text="Вариант 3", foreground="black")
        elif v_s[3] == 1:
            im = app_7_6_18.config_path + "/Images/Indication_7_6_18/Var4.png"
            self.result_lbl.config(text="Вариант 4", foreground="black")
        elif v_s[6] == 1:
            im = app_7_6_18.config_path + "/Images/Indication_7_6_18/Var7.png"
            self.result_lbl.config(text="Вариант 7", foreground="black")
        else:
            im = app_7_6_18.config_path + "/Images/Indication_7_6_18/Var8.png"
            self.result_lbl.config(text="Ничего не отображается", foreground="black")
        return im

    def screen(self):
        cap = tkcap.CAP(self.win)
        cap.capture(app_7_6_18.config_path + "/Screenshots/Screen{}.png".format(str(self.count)))
        self.count += 1

    def copy(self):
        pyperclip.copy(app_7_6_18.logic)
        return 0

    def __init__(self, master, txt_results=None):
        self.win = master
        # Create window
        self.win.title("Логика индикации вертикальных режимов AP/FD")
        ...
        style.map("C.TFrame", foreground="black")

        # Label menu
        self.img1 = Image.open(app_7_6_18.config_path + "/logo.png")
        self.img2 = Image.open(app_7_6_18.config_path + "/ssj1.png")
        self.img3 = Image.open(app_7_6_18.config_path + "/ssj2.png")
        ...
        for i in range(len(app_7_6_18.descriptSig)):
            ...
            exec('self.text{}.insert(1.0, "{}")'.format(str(i), app_7_6_18.descriptSig[i]))
            ...
            exec('self.lbl{} = Label(self.fr{}, text="{}", font=("Times New Roman", 11), width=25, height=2)'.format(
                str(i), str(i), app_7_6_18.nameSig[i]))
            ...
            exec('self.text{}1 = Text(self.fr{}, font=("Times New Roman", 11), height=2, width=10, wrap=WORD, bg="#ececec")'.format(
                str(i), str(i)))
            exec('self.text{}1.tag_configure("center", justify="center")'.format(str(i)))
            exec('self.text{}1.insert(1.0, "{}")'.format(str(i), app_7_6_18.rangeSig[i]))
        ...
        self.text = Text(self.fr_logic, font=("Times New Roman", 12), width=70, yscrollcommand=self.scr.set, foreground='black')
        self.text.insert(END, app_7_6_18.logic)
        ...
        # Добавим изображение
        self.c_image = self.canvas.create_image(9, 9, image=self.photo, anchor='nw')
