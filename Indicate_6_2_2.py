...
class app_6_2_2:
    ...
    # Ввод переменных
    xl_new = read_excel(config_path + '/logic.xlsx', sheet_name='6_2_2')
    ...
    im = config_path + '/Images/Indication_6_2_2/Var4.png'

    logic = open(config_path + '/logic_text/logic_6_2_2.txt', encoding='utf-8').readlines()
    logic = ''.join(logic)
    ...
    def chek_b1(self):
        v_s = []
        p_s = []
        for l in range(len(app_6_2_2.descriptSig)):
            exec('v_s.append(self.var{}.get())'.format(str(l)))
            exec('p_s.append(self.txt{}.get())'.format(str(l)))
        print(v_s, p_s, sep="\n")

        self.im = app_6_2_2.logik_of_indication(self, v_s, p_s)
        print(self.im)
    ...
    def logik_of_indication(self, v_s, p_s):
        if v_s == [1, 1, 1, 1, 1, 1]:
            if p_s[1] == 1 and p_s[3] == 1 and p_s[5] == 1:
                if (p_s[0] + p_s[2] + p_s[4]) / 3 >= -5:
                    im = app_6_2_2.config_path + "/Images/Indication_6_2_2/Var1.jpeg"
                    self.result_lbl.config(text="Вариант 1", foreground="black")
                elif (p_s[0] + p_s[2] + p_s[4]) / 3 <= -50:
                    im = app_6_2_2.config_path + "/Images/Indication_6_2_2/Var5.jpeg"
                    self.result_lbl.config(text="Вариант 1\nВысота стрелки соответствует\nмаксимальному значению", foreground="black")
                else:
                    im = app_6_2_2.config_path + "/Images/Indication_6_2_2/Var3.png"
                    self.result_lbl.config(text="Вариант 3", foreground="black")
            elif (p_s[0] + p_s[2] + p_s[4]) / 3 >= -5:
                im = app_6_2_2.config_path + "/Images/Indication_6_2_2/Var2.jpeg"
                self.result_lbl.config(text="Вариант 2", foreground="black")
            elif (p_s[0] + p_s[2] + p_s[4]) / 3 <= -50:
                im = app_6_2_2.config_path + "/Images/Indication_6_2_2/Var6.jpeg"
                self.result_lbl.config(text="Вариант 6\nВысота стрелки соответствует\nмаксимальному значению", foreground="black")
            else:
                im = app_6_2_2.config_path + "/Images/Indication_6_2_2/Var4.png"
                self.result_lbl.config(text="Вариант 4", foreground="black")
        else:
            im = app_6_2_2.config_path + "/Images/Indication_6_2_2/Var7.jpeg"
            self.result_lbl.config(text="Вариант 7", foreground="black")
        return im

    def screen(self):
        cap = tkcap.CAP(self.win)
        cap.capture(app_6_2_2.config_path + "/Screenshots/Screen{}.png".format(str(self.count)))
        self.count += 1

    def copy(self):
        pyperclip.copy(app_6_2_2.logic)
        return 0
    ...
    def __init__(self, master, txt_results=None):
        ...
        # Label menu
        self.img1 = Image.open(app_6_2_2.config_path + "/logo.png")
        self.img2 = Image.open(app_6_2_2.config_path + "/ssj1.png")
        self.img3 = Image.open(app_6_2_2.config_path + "/ssj2.png")

        self.width = 400

        self.ratio = (self.width / 3000)
        self.height = int((float(self.img1.size[1]) * float(self.ratio)))
        self.imag1 = self.img1.resize((300, self.height), Image.Resampling.LANCZOS)
        self.image1 = ImageTk.PhotoImage(self.imag1)

        self.imag2 = self.img2.resize((self.width, 150), Image.Resampling.LANCZOS)
        self.image2 = ImageTk.PhotoImage(self.imag2)

        self.imag3 = self.img3.resize((self.width, 150), Image.Resampling.LANCZOS)
        ...
        for i in range(len(app_6_2_2.descriptSig)):
            ...
            exec('self.text{}.insert(1.0, "{}")'.format(str(i), app_6_2_2.descriptSig[i]))
            ...
            exec('self.lbl{} = Label(self.fr{}, text="{}", font=("Times New Roman", 11), width=16, height=2)'.format(
                str(i), str(i), app_6_2_2.nameSig[i]))
            ...
            exec('self.text{}1.insert(1.0, "{}")'.format(str(i), app_6_2_2.rangeSig[i]))
        ...
        self.text.insert(END, app_6_2_2.logic)
